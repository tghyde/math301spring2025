...
theorem infinite_surjective
  ...
  (hY : is_infinite Y)
  : is_infinite X := by
    cases' surjective_has_right_inverse f hf with f' hf'
    have hf'_inj : is_injective f' := by
      apply has_left_inverse_injective
      use f
      intro y
      exact hf' y
      done
    cases' hY with g hg
    use (f' ∘ g)
    exact comp_injective hg hf'_inj
    done
...
theorem finite_surjective
  ...
  : is_finite Y := by
    intro hY
    apply hX
    exact infinite_surjective f hf hY
    done
...
theorem finite_injective
  ...
  : is_finite X := by
    intro hX
    apply hY
    exact infinite_injective f hf hX
    done
