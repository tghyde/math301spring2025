...
example (A₁ A₂ : Set X) : f '' (A₁ ∪ A₂) = (f '' A₁) ∪ (f '' A₂) := by
  ...
  · rintro (⟨x,h1,hx⟩ | ⟨x,h2,hx⟩)
    · use x
      exact ⟨by left; assumption, hx⟩
      done
    · use x
      exact ⟨by right; assumption, hx⟩
      done
    done
  done
...
example : ∃ (X Y : Type) (A₁ A₂ : Set X) (f : X → Y),
  ...
  have h₁ : const '' ({0} ∩ {1}) = (∅ : Set ℕ) := by
    rw [h₀]
    exact Set.image_empty const
    done
  have h₂ : const '' {0} = {0} := by exact Set.image_singleton
  have h₃ : const '' {1} = {0} := by exact Set.image_singleton
  intro h
  rw [h₁, h₂, h₃] at h
  have h₄ : 0 ∈ (∅ : Set ℕ) := by
    rw [h]
